...
const btnCalculate = document
  .querySelector("#btnCalculate")
  .addEventListener("click", calculate)
document.querySelector("#btnNext").addEventListener("click", checkMyDay)
document.querySelector("#btnCheckMyDay").addEventListener("click", checkMyDay)
document.querySelector("#btnTracking").addEventListener("click", tracking())
document.querySelector("#selectGrains").addEventListener("click", addFood(this))
document
  .querySelector("#selectVegetables")
  .addEventListener("click", addFood(this))
document.querySelector("#selectFruits").addEventListener("click", addFood(this))
...
function logIn() {
  ...
  document.getElementById("personalGreeting").innerHTML = "HI  !"

  //   document.getElementById("personalQuestion").innerHTML =
...
function showSelect() {
  ...
  if (sexValue === "female") {
    document.getElementById("selectPregnantLactating").style.display =
      "inline-block"
...
function calculate() {
  ...
  let pregnantOrLactating = document.getElementById(
    "selectPregnantLactating"
  ).value
  ...
  if (age === "" || sex === "") {
    ...
    document.getElementById("messageFillIn2").innerHTML =
      "This app is not for users under 18 years"
  }
  ...
  //If the form is filled, calculate
    ...
    if (sex === "male") {
      ...
      reqCalories =
        662 - 9.53 * age + activity * (15.91 * weight + (539.6 * height) / 100)
      ...
      reqCalories =
        354 - 6.91 * age + activity * (9.36 * weight + (726 * height) / 100)
...
function tracking() {
  ...
  if (tableList.rows.length === 1) {
    document.getElementById("messageFillIn3").innerHTML =
    ...
    for (let indexRows = 1; indexRows < tableList.rows.length; indexRows++) {
      ...
      totalNutrients["Calories"] +=
        (foodCtg[foodValue]["Calories"] * foodQuantity) / 100
      totalNutrients["TotalCarbohydrate"] +=
        (foodCtg[foodValue]["TotalCarbohydrate"] * foodQuantity) / 100
      totalNutrients["LinoleicAcid"] +=
        (foodCtg[foodValue]["LinoleicAcid"] * foodQuantity) / 100
      totalNutrients["AlphaLinoleicAcid"] +=
        (foodCtg[foodValue]["AlphaLinoleicAcid"] * foodQuantity) / 100
      totalNutrients["Protein"] +=
        (foodCtg[foodValue]["Protein"] * foodQuantity) / 100
      totalNutrients["DietaryFiber"] +=
        (foodCtg[foodValue]["DietaryFiber"] * foodQuantity) / 100
      totalNutrients["VitA"] +=
        (foodCtg[foodValue]["VitA"] * foodQuantity) / 100
      totalNutrients["VitD"] +=
        (foodCtg[foodValue]["VitD"] * foodQuantity) / 100
      totalNutrients["VitE"] +=
        (foodCtg[foodValue]["VitE"] * foodQuantity) / 100
      totalNutrients["VitK"] +=
        (foodCtg[foodValue]["VitK"] * foodQuantity) / 100
      totalNutrients["VitC"] +=
        (foodCtg[foodValue]["VitC"] * foodQuantity) / 100
      totalNutrients["VitB1"] +=
        (foodCtg[foodValue]["VitB1"] * foodQuantity) / 100
      totalNutrients["VitB2"] +=
        (foodCtg[foodValue]["VitB2"] * foodQuantity) / 100
      totalNutrients["VitB3"] +=
        (foodCtg[foodValue]["VitB3"] * foodQuantity) / 100
      totalNutrients["VitB5"] +=
        (foodCtg[foodValue]["VitB5"] * foodQuantity) / 100
      totalNutrients["VitB6"] +=
        (foodCtg[foodValue]["VitB6"] * foodQuantity) / 100
      totalNutrients["VitB9"] +=
        (foodCtg[foodValue]["VitB9"] * foodQuantity) / 100
      totalNutrients["VitB12"] +=
        (foodCtg[foodValue]["VitB12"] * foodQuantity) / 100
    ...
    for (let rowIndex = 0; rowIndex < tableResult.rows.length; rowIndex++) {
      if (rowIndex == 6 || rowIndex == 19) {
        ...
        tableResult.rows.item(rowIndex).cells[1].children[0].innerHTML =
          percents.toFixed(0) + "%"

        if (percents >= 100) {
          tableResult.rows.item(rowIndex).cells[1].children[0].style.width =
            "100%"
          tableResult.rows.item(
            rowIndex
          ).cells[1].children[0].style.backgroundColor = "#8aff75"
        } else {
          tableResult.rows.item(rowIndex).cells[1].children[0].style.width =
            percents.toFixed(0) + "%"
          tableResult.rows.item(
            rowIndex
          ).cells[1].children[0].style.backgroundColor = "#ff758a"
        }
      }
    }
  }
